...
# Constants
LATENT_DIM = 5
shap_files = [f"{shap_dir}/latent_{i}_shap_ai4i.csv" for i in range(LATENT_DIM)]
...
def plot_latent_shap_heatmap():
    ...
    plt.title("Latent-Feature SHAP Attribution Heatmap (AI4I Dataset)")
    plt.xlabel("Features")
    plt.ylabel("Latent Dimensions")
    plt.tight_layout()
    plt.savefig(f"{output_dir}/latent_shap_heatmap_ai4i.png")
    plt.close()
...
def plot_cumulative_shap_contribution():
    ...
    plt.title("Cumulative Feature Attribution per Latent (AI4I)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/cumulative_shap_contribution_ai4i.png")
    plt.close()
...
def plot_feature_top3_dominance():
    ...
    plt.title("Feature Frequency in Top-3 Latent SHAP Attributions (AI4I)")
    plt.ylabel("Count (out of 10 latents)")
    plt.xlabel("Feature")
    plt.tight_layout()
    plt.savefig(f"{output_dir}/feature_top3_dominance_ai4i.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    print("🎉 All SHAP visualizations for AI4I completed.")
